...
int isBlack(struct Node* No)
{
	return (No == NULL) || (No->color == BLACK);
}

int isRed(struct Node* No)
{
...
int Altura(struct Node*  r)
{
   ...
   if( r != NULL )
   {
       AlturaE = Altura(r->left);
       AlturaD = Altura(r->right);
...
int calculaAltura(struct Node *raiz){
    if(raiz == NULL){
        return 0;
    }
    else {
        return (maior(calculaAltura(raiz->left),calculaAltura(raiz->right)) + 1);
    }
}
...
int main()
{
    ...
    struct Node* x = BST_search(RBT,remove);
